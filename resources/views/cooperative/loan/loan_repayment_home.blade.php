            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0 font-size-18">Member Loan Repayment</h4>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header border-bottom d-flex">
                            <h4 class="card-title">Member Loan Repayment</h4>
                            <button type="button" class="btn btn-primary waves-effect waves-light ms-auto"
                                id="edit-employee" data-bs-toggle="modal" data-bs-target="#addModal">Repayment</button>
                        </div>
                                ...
                                <tbody>
                                    @foreach ($payments as $payment)
                                        <tr>
                                            ...
                                            <td>{{ $payment->bank() }}</td>
                                            <td style="text-align: right">{{ 'N' . number_format($payment->debit, 2) }}</td>
                                            <td>{{ $payment->date }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                <div class="card-body p-4">
                    <form method="POST" id="frm_main" onsubmit='disableButton()'>
                        ...
                        <div class="row">
                            ...
                            <input type="hidden" name="is_loan" value="1">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary mr-2 submit-btn" name="save" id="btn">Save<span
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".member").on("change", function(e) {
                ...
                $.ajax({
                    url: '{{ route('get_member_loans') }}?id=' + id,
                    type: "GET",
                    dataType: "json",
                    success: function(response) {
                        ...
                        if (len > 0) {
                            ...
                            for (var i = 0; i < len; i++) {
                               ...
                               ${!response['data'][i]?.loan_name ? '' : response['data'][i]?.ln?.description}  -   ${response['data'][i].balance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}                  
                            </option>`
                                );
                            }
                        }
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#frm_main").on('submit', async function(e) {
                ...
                try {
                    const postRequest = await request("/admin/member-repayment/transact",
                        processFormInputs(
                            serializedData), 'post');
